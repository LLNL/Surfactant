...
from surfactant.input_readers.cytrics_reader import SBOM
...
class SelectFile(textual.screen.ModalScreen[Optional[textual.widgets.DirectoryTree.FileSelected]]):
   ...
   def on_directory_tree_file_selected(self, path: textual.widgets.DirectoryTree.FileSelected) -> None:
      self.dismiss(path)
   ...
   def on_archive_button(self):
      def set_archive(path: Optional[textual.widgets.DirectoryTree.FileSelected]):
         if path:
            ...
            self.get_child_by_id('archive_label', textual.widgets.Label).update(f'Archive: {loc}')
...
class ConfigEntry(textual.widgets.Static):
   ...
   def on_delete_entry(self):
      ...
      self.app.push_screen(YesNoScreen('Are you sure you want to delete this entry?'), delete_self)
...
class AddEntry(textual.widgets.Static):
   ...
   def on_add_config_entry(self):
      ...
      add_entries.mount(textual.widgets.Button(str(self.app.entry_id), id=f'view_entry_{self.app.entry_id}'))
...
class MainBody(textual.widgets.Static):
   ...
   def compose(self) -> textual.app.ComposeResult:
      ...
      yield textual.containers.ScrollableContainer(AddEntry(id='add_entry'), id='add_entry_scrollable')
   ...
   def on_button_pressed(self, event: textual.widgets.Button.Pressed) -> None:
      ...
      if button_id := event.button.id:
         if button_id.startswith(START_BUTTON_TEXT):
            ...
            try:
               self.app.query_one(f'#{START_BUTTON_TEXT}{self.selected_button_num}', textual.widgets.Button).remove_class('selected')
            except textual.css.query.NoMatches:
               pass
            event.button.add_class('selected')
            entry = self.app.query_one('#config_entries', textual.containers.ScrollableContainer)
